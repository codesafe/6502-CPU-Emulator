...
GuiFileDialogState InitGuiFileDialog(int width, int height, const char *initPath, bool active)
{
	...
	Vector2 v0 {
		GetScreenWidth() / 2 - state.size.x / 2, GetScreenHeight() / 2 - state.size.y / 2
	};
    state.position = v0;
...
void GuiFileDialog(GuiFileDialogState *state)
{
    if (state->fileDialogActive)
    {
        ...
        Rectangle r0{
            state->position.x + 0, state->position.y + 0, winWidth, winHeight
        };

        state->fileDialogActive = !GuiWindowBox(r0, "#198# Select File Dialog");

        Rectangle r2{
            state->position.x + winWidth - 50, state->position.y + 35, 40, 25
        };
        if (GuiButton(r2, "< ..")) // || IsKeyReleased(KEY_DPAD_Y))
        {
		...
		Rectangle r1 {
			state->position.x + 10, state->position.y + 35, winWidth - 65, 25
        };
        if (GuiTextBox(r1, state->dirPathText, 256, state->dirPathEditMode))
        {
...
# else
        ...
        Rectangle r3{
            state->position.x + 10, state->position.y + 70, winWidth - 20, winHeight - 135
        };
        state->filesListActive = GuiListViewEx(r3, (const char**)dirFilesIcon, state->dirFilesCount, &state->itemFocused, &state->filesListScrollIndex, state->filesListActive);
# endif
        ...
        Rectangle r5{
            state->position.x + 10, state->position.y + winHeight - 60, 68, 25
        };
        GuiLabel(r5, "File name:");

        Rectangle r6{
            state->position.x + 75, state->position.y + winHeight - 60, winWidth - 200, 25
        };
        if (GuiTextBox(r6, state->fileNameText, 128, state->fileNameEditMode))
        {
        ...
        Rectangle r7{
            state->position.x + 75, state->position.y + winHeight - 30, winWidth - 200, 25
        };
        state->fileTypeActive = GuiComboBox(r7, "All files", state->fileTypeActive);

        Rectangle r9{
            state->position.x + 10, state->position.y + winHeight - 30, 68, 25
        };
        GuiLabel(r9, "File filter:");


        Rectangle r8{
            state->position.x + winWidth - 120, state->position.y + winHeight - 60, 110,
        ...
        state->SelectFilePressed = GuiButton(r8, "Select");// || IsKeyPressed(KEY_ENTER) || IsKeyPressed(KEY_DPAD_A);
