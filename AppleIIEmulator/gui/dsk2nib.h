...
bool LoadDskFile(char* path, unsigned char* buffer);
...
void dsk_init(void)
{
	...
	for (i = 0; i < TRACKS_PER_DISK; i++)
		if ((dsk_buf[i] = (uchar*)malloc(BYTES_PER_TRACK)) == NULL)
			printf("cannot allocate %ld bytes", DSK_LEN);
}
...
void dsk_read(char* path)
{
	...
	for (i = 0; i < TRACKS_PER_DISK; i++)
		if (fread(dsk_buf[i],1, BYTES_PER_TRACK,fd) != BYTES_PER_TRACK)
			printf("dsk write failure");
...
void nib_init(void)
{
	...
	for (i = 0; i < TRACKS_PER_DISK; i++)
		if ((nib_buf[i] = (uchar*)malloc(BYTES_PER_NIB_TRACK)) == NULL)
			printf("cannot allocate %d bytes", BYTES_PER_NIB_TRACK);
}
...
void nib_write(char* path)
{
	...
	for (i = 0; i < TRACKS_PER_DISK; i++)
		if (fwrite(nib_buf[i], 1, BYTES_PER_NIB_TRACK,fd) !=
			BYTES_PER_NIB_TRACK)
			printf("nib write error");
...
bool LoadDskFile(char* path, unsigned char* buffer)
{
	...
	{
		memcpy(buffer + pos, nib_buf[i], BYTES_PER_NIB_TRACK);
		pos += BYTES_PER_NIB_TRACK;
	}
